...
export function ResultView({ trip }: { trip: Trip }) {
  const [activeKind, setActiveKind] = useState<AlternativeKind>("cheapest");
  const active = useMemo(
    () => trip.alternatives.find((a) => a.kind === activeKind) ?? trip.alternatives[0],
    [trip, activeKind]
  );
  ...
  // Sequence villes (start + dests dans l'ordre)
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-6 pb-24 pt-8">
        ...
        <div className="grid grid-cols-12 gap-6 items-end">
          <div className="col-span-12 md:col-span-8">
            ...
            <p className="mt-4 max-w-xl text-[15px] leading-[1.55] text-ink-soft">
              ...
              départ de {trip.startCity.name}. Trois façons d&apos;arriver là-bas —
              sélectionnez celle qui vous ressemble.
            </p>
          </div>
          ...
          {trip.alternatives.map((alt) => {
            ...
            return (
              <button
                ...
                className="group relative text-left rounded-[16px] border bg-surface px-5 py-4 transition-all duration-200 ease-out hover:-translate-y-0.5 active:scale-[0.99]"
                style={{
      ...
      <section className="mb-10 grid grid-cols-12 gap-6">
        <div className="col-span-12 lg:col-span-7">
          ...
          <p className="mt-2.5 text-[11.5px] text-ink-mute">
            ...
            <span style={{ color: activeMeta.accentInk }}>{activeMeta.label.toLowerCase()}</span>.
          </p>
        </div>
          ...
          <div className="rounded-[18px] border border-line bg-surface p-6">
            ...
            <ol className="mt-5 space-y-4">
              {sequenceCities.map((city, idx) => {
                if (!city) return null;
                const dest = trip.destinations.find((d) => d.city.name === city.name);
                const isStart = idx === 0;
                return (
                  <li key={`${city.name}-${idx}`} className="flex items-start gap-3">
                    <div
                      className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px]"
                      style={{
                        background: isStart ? activeMeta.accentInk : activeMeta.accentSoft,
                        color: isStart ? activeMeta.accentSoft : activeMeta.accentInk,
                        fontFamily: "var(--font-mono)",
                      }}
                    >
                      {idx + 1}
        ...
        <div className="overflow-hidden rounded-[18px] border border-line bg-surface">
          <ul className="divide-y divide-line">
            {active.legs.map((leg, idx) => {
              const isLast = idx === active.legs.length - 1;
              return (
                <li
      ...
      <section className="mb-10">
        <div
          className="rounded-[24px] border px-8 py-8 grid grid-cols-12 gap-6 items-center"
          style={{
            background: activeMeta.accentSoft,
        ...
        >
          <div className="col-span-12 md:col-span-7">
            ...
            <p className="mt-3 max-w-md text-[14px] text-ink-soft">
              ...
              {formatDateLong(trip.startDate)} et {formatDateLong(trip.endDate)}, au départ
              de {trip.startCity.name}.
            </p>
          </div>
          <div className="col-span-12 md:col-span-5 grid grid-cols-3 gap-4">
            <Stat label="Total" value={formatCost(active.totalCostEUR)} ink={activeMeta.accentInk} />
            <Stat
              label="Durée"
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ fontFamily: "var(--font-mono)", fontWeight: 500, color: ink }}
      >
        {value}
      </div>
    </div>
